...
@njit
def check_smooth_border(labelling,Q):
    height,width = labelling.shape
    smooth_border = False
    for i in range(1,height-1):
        for j in range(width):
            if labelling[i, j] == 0:
                if labelling[i + 1, j] == 1:
                    if labelling[i-1, j] == 1:
                        smooth_border = True
                        Q[i,j,1] += 1
                    elif labelling[i-1, j] == 0:
                        smooth_border = True
                        Q[i+1,j,0] += 1
                elif labelling[i + 1, j] == 2:
                    if labelling[i - 1, j] == 2:
                        smooth_border = True
                        Q[i,j,2] += 1
                    elif labelling[i - 1, j] == 0:
                        smooth_border = True
                        Q[i+1,j,0] += 1
            if labelling[i, j] == 1 and labelling[i + 1, j] == 2:
                    if labelling[i - 1, j] == 2:
                        smooth_border = True
                        Q[i,j, 2] += 1
                    elif labelling[i - 1, j] == 1:
                        smooth_border = True
                        Q[i+1,j,1] += 1
    return smooth_border, Q

@njit
def check_cons(labelling,Q):
    height,width = labelling.shape
    consistent = False
    for i in range(height-1):
        for j in range(width):
            if labelling[i, j] == 0 and labelling[i+1, j] == 0:
                if j > 0:
                    if (labelling[i, j-1] == 1 and labelling[i+1, j-1] == 1) or (labelling[i, j-1] == 2 and labelling[i+1, j-1] == 2):
                        consistent = True
                        Q[i,j,1] += 1
                if j < width - 1:    
                    if (labelling[i, j+1] == 1 and labelling[i+1, j+1] == 1) or (labelling[i, j+1] == 2 and labelling[i+1, j+1] == 2):
                        consistent = True
                        Q[i,j,1] += 1
            if labelling[i, j] == 1 and labelling[i+1, j] == 1:
                if (j > 0 and labelling[i, j-1] == 2 and labelling[i+1, j-1] == 2) or (j < width - 1 and labelling[i, j+1] == 2 and labelling[i+1, j+1] == 2):
                    consistent = True
                    Q[i,j,2] += 1
    return consistent, Q


def get_gmm_params(img, bbox0, bbox1, bbox2):
    class0 = img[bbox0[0]:bbox0[1], bbox0[2]:bbox0[3]].reshape(-1, 3)
    class1 = img[bbox1[0]:bbox1[1], bbox1[2]:bbox1[3]].reshape(-1, 3)
    class2 = img[bbox2[0]:bbox2[1], bbox2[2]:bbox2[3]].reshape(-1, 3)

    class0_gmm = GaussianMixture(n_components=1).fit(class0)
    class1_gmm = GaussianMixture(n_components=1).fit(class1)
    class2_gmm = GaussianMixture(n_components=1).fit(class2)
    
    params = (class0_gmm.means_[0], class0_gmm.covariances_[0],
              class1_gmm.means_[0], class1_gmm.covariances_[0],
              class2_gmm.means_[0], class2_gmm.covariances_[0])
    return params


def get_g(height,width,n_labels,beta):
    g = np.zeros((height,width, 4, n_labels, n_labels))
    for k in range(n_labels):
        for k1 in range(n_labels):
            g[..., k, k1] = -beta*(k!=k1)
    return g
...
def get_q(img, mean0, cov0, mean1, cov1, mean2, cov2):
    height, width = img.shape[:2]
    K = np.array([0,1,2])
    mask01 = multivariate_normal.pdf(img, mean0, cov0) > multivariate_normal.pdf(img, mean1, cov1)
    mask02 = multivariate_normal.pdf(img, mean0, cov0) > multivariate_normal.pdf(img, mean2, cov2)
    mask12 = multivariate_normal.pdf(img, mean1, cov1) > multivariate_normal.pdf(img, mean2, cov2)
